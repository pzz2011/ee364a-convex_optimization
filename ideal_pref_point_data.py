...
c_ideal = Variable(n)
cons = [c_ideal >= 0, c_ideal <= 1]
for dcs in d:
    i = dcs[0]
    j = dcs[1]
    ci = np.asmatrix(c[i]).T
    cj = np.asmatrix(c[j]).T
    cons += [ci.T * ci - 2 * ci.T * c_ideal <= cj.T * cj - 2 * cj.T * c_ideal]

for i in range(n):
    prob = Problem(Minimize(c_ideal[i]), cons)
    prob.solve()
    box[i][0] = prob.value

for i in range(n):
    prob = Problem(Maximize(c_ideal[i]), cons)
    prob.solve()
    box[i][1] = prob.value
